...
def profile():
    ...
    return render_template("profile.html", user=current_user, img=img, posts=posts, comments = comments)
...
def upload():
    ...
    filename = secure_filename(file.filename)
    # Use a relative path within the container
    upload_folder = os.path.join(current_app.root_path, 'static', 'photos', 'profile_pictures')
...
def update():
    ...
    elif len(password) < 5 and password:
